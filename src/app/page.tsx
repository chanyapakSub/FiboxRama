...
export default function LandingPage() {
  ...
  const [view, setView] = useState<"main" | "admin" | "login" | "register" | "forgot_password">("main");
  ...
  if (view === "forgot_password") {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 py-10">
        <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md border border-slate-200">
          <button onClick={() => setView("login")} className="text-sm text-slate-500 mb-4 hover:text-slate-800">← Back to Login</button>
          <h2 className="text-2xl font-bold mb-2 text-slate-800">Reset Password</h2>
          <p className="text-slate-500 mb-6 text-sm">Verify your identity to reset password</p>
          ...
          <form onSubmit={handlePasswordReset} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Full Name (used for verification)</label>
              <input
                required
                type="text"
                className="w-full p-3 border border-slate-300 rounded focus:ring-2 focus:ring-blue-500 outline-none"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-slate-800 text-white py-4 rounded-lg hover:bg-slate-900 font-bold shadow-md mt-6 disabled:opacity-50"
            >
